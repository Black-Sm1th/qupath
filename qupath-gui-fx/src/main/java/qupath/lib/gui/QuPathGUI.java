...
public class QuPathGUI {
	...
	private Scene createAndInitializeMainScene(Parent content) {
		...
		try {
			...
			var width = bounds.getWidth() * 0.8;
			var height = bounds.getHeight() * 0.8;
	...
	private Menu createRecentProjectsMenu() {
		...
		Menu menuRecent = GuiTools.createRecentItemsMenu("最近的项目...", recentProjects, uri -> {
			Project<BufferedImage> project;
			try {
				project = ProjectIO.loadProject(uri, BufferedImage.class);
				setProject(project);
			} catch (Exception e1) {
				Dialogs.showErrorMessage("项目错误", "未能找到项目 " + uri);
